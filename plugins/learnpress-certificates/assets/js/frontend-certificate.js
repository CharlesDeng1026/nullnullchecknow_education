...
(function ($) {
	...
	function loadImage(src, callback) {
		...
		var data = canvas.toSVG();		var locfile = new Blob([data], {type: "image/svg+xml;charset=utf-8"});
		...
		loadImage(certificate.url, function (dimensions) {			frame.ratio = frame.width / dimensions.width;
			...
			fabric.Image.fromURL(certificate.url, function (img) {
				...
				var args = this;
				if (!args.type) return;				$.each(['fontSize', 'top', 'left'], function () {
			...
			}, 150);			$('.cert-download-png', $cert).unbind('click').click(function (e) {
				e.preventDefault();
